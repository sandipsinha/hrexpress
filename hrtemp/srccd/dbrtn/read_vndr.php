...
$().ready(function() {
          $('#getbtn').hover( function(){
           $('#getbtn').attr('value','Click here to search, update,correct  a Vendor ')
           },
        function(){
...
$("#getbtn").click(function(event){  
   event.preventDefault();
    var strid = $("#vndrid").val();
   $.ajax({type:"POST",
    url:"vndr_dtls.php",
    data:({datastring:strid}),
    success:function(response){
        document.location = 'mntnvndr.php';
        }
   });
...
- Tracking Tool</h2>
<br>
<p id='headtext'>Enter a Vendor name to search </p>
	<form autocomplete="off" id="target">
		<div class="subfieldsset">
            ...
            <input type="button" name="addbtn" class="button" id="addbtn" value="Add" />  
            <div>
			<input type="hidden" name="id" id="vndrid" />
		</div>
        <br>
        </div></div>
        <span id="results">
